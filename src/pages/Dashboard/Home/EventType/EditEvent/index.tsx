...
import { getEventByEventId } from '@/lib/utils'
...
const EditEventPage = () => {
  ...
  useEffect(() => {
    const getEvent = async () => {
      const res = await getEventByEventId(eventId, 'regular')
      if (res.success) {
        form.reset({
          eventName: res.event.eventName,
          eventDuration: res.event.eventDuration,
          eventDateRange: res.event.eventDateRange,
        })
        setLocationType(res.event.locationType)
        setLocationNote(res.event.locationNote)
      }
    }
    getEvent()
  ...
  const onSubmit = async (values: z.infer<typeof EventSchema>) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/user-event/${eventId}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="size-full flex flex-col">
      ...
      <h1 className=" text-lg px-3 pb-3 font-semibold">Modify the Event</h1>
